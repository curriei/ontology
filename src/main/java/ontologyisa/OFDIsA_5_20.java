...
public class OFDIsA_5_20 {
	...
	private static Map<Vertex, Vertex> bfsCar = BFS(IO.createMedicinesGraph(), MEDICINE);
	...
	public static void setExcessThree(boolean isExcessThree) {
		OFDIsA_5_20.isExcessThree = isExcessThree;
	}
	...
	public static void main(String[] args) throws SQLException, IOException {
		...
		writer = new PrintWriter("isa_8_100", "UTF-8");

		processList(iteqmap, itcmap);
		
		writer.close();
		
		stripDuplicatesFromFile("isa_8_100");
		//System.out.println();
		
		System.out.println("contents of file are: ");
		System.out.println();
		
		BufferedReader br = new BufferedReader(new FileReader("isa_8_100"));
		 String line = null;
	...
	private static boolean compareGreaterLength(Object[] array, String ckey) {
		...
		if (compare.size() == 0)
			...
			boolean value = isa.isWithinThreshold(isa.findLCA(diseaseList), diseaseList);
			return value;
		} else if (ckey == "medicine") {
			List<Vertex> medicineList = new ArrayList<Vertex>();
			for (Vertex vertex : vertexlist) {
				medicineList.add(bfsCar.get(vertex));
			}
